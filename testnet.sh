...
echo -e "${BLUE}Enter ether amount you want to deposit in Eclipse Testnet (Min: 0.002 ETH):${NC}"
read ETH_AMOUNT
echo
...
for i in "${!private_key_array[@]}"; do
  ...
  echo -e "${GREEN}You can check Eclipse Deposit Tx here: https://explorer.dev.eclipse.xyz/address/$address$?cluster=testnet{NC}"
  echo
done
